...
## Persona distribution model

- `matraix/personal-persona/persona.example.yaml` is the tracked blank template. It contains all 1,290 schema dimension IDs with every value set to `null`, so the repository contains no filled personal profile.
- `matraix/personal-persona/persona.yaml` is the private filled runtime file. It is ignored by Git.
- `install.sh` and `start-local.sh` copy the tracked template to `persona.yaml` only when no active file exists. They never overwrite an existing `persona.yaml`.
- Persona compilation and Update persona fill the private runtime file while leaving the tracked template unchanged.
...
The filled persona, candidate evidence, survey answers, Validation results, logs, and Python environment stay in the local checkout and are ignored by Git. Only the all-null `persona.example.yaml` template is tracked. The server binds only to `127.0.0.1` or `localhost`.
